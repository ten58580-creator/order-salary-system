-- Update get_daily_production_v2 to use scheduled_date instead of order_date
-- This allows the postpone feature to work correctly
...
BEGIN
    ...
    WITH order_data AS (
        ...
        WHERE o.scheduled_date = p_target_date  -- Changed from order_date to scheduled_date
    ),
    summary AS (
        SELECT
            od.product_id,
            ...
            (
                ...
                WHERE sub.product_id = od.product_id AND sub.scheduled_date = p_target_date  -- Changed
            ) as current_status,
            BOOL_OR(od.is_correction) as has_correction,
            MIN(od.display_order) as display_order  -- Get the minimum display_order for the product
        FROM order_data od
        ...
        COALESCE(
            (
                ...
                FROM (
                    ...
                    FROM public.orders sub
                    WHERE sub.product_id = s.product_id AND sub.scheduled_date = p_target_date  -- Changed
                    GROUP BY status
                ) sub_counts
            ),
            '{}'::jsonb
            ...
            (
                SELECT jsonb_agg(
                    jsonb_build_object(
                        ...
                        'status', od.status,
                        'is_correction', od.is_correction
