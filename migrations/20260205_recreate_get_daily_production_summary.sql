-- ■ get_daily_production_summary 完全再作成SQL
-- 404エラー (関数が見つからない、シグネチャ不一致) と RPCエラー (NULL集計エラー) を同時に解決します。

-- 1. 既存の関数を削除 (念のため引数違いも削除)
DROP FUNCTION IF EXISTS get_daily_production_summary(date);
DROP FUNCTION IF EXISTS get_daily_production_summary(p_target_date date);
...
-- 3. キャッシュリフレッシュのためのダミー通知 (Supabaseコンソールで実行すれば自動反映されます)
